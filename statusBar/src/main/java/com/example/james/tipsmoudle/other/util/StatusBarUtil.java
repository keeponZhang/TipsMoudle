package com.example.james.tipsmoudle.other.util;
...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            Log.e("TAG", "StatusBarUtil setColor KITKAT:"+decorView);
    ...
    public static void setTransparentNoFixSystemWindow(Activity activity) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        transparentStatusBar(activity);
    }
    public static void setTransParentWithUIFlag(Activity activity){
...
//			最后再调用Window的setStatusBarColor()方法将状态栏设置成透明色就可以了。
    ...
    /**
     * 设置根布局参数 其实就是setFixSystemWindow
     */
    private static void setRootView(Activity activity) {
        ViewGroup parent = (ViewGroup) activity.findViewById(android.R.id.content);
        for (int i = 0, count = parent.getChildCount(); i < count; i++) {
            View childView = parent.getChildAt(i);
            Log.e("TAG", "StatusBarUtil setRootView childView:"+childView);
    ...
    private static void transparentStatusBar(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            // activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            //FLAG_TRANSLUCENT_NAVIGATION也会造成布局与navigationbar位置重叠
            // activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
            //设置其他颜色才要清除WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS的flag
